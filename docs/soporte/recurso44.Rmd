...
```
**Figura 1.50** Proporción y patrón de datos faltantes.
</center>
</br>

La gráfica de la derecha de la **Figura 1.50** muestra el porcentaje de datos faltantes en cada variable. También, se presenta en la misma figura los 21 patrones de datos faltantes presentes en la base de datos. El color azul indica los datos observados, mientras que el rojo señala los datos faltantes.

En la **Figura 1.50**, en el primer gráfico, se observa que el porcentaje de universidades con observaciones faltantes es significativamente mayor en las variables ACT y SAT en comparación con las demás. En el segundo gráfico, se muestran las combinaciones de observaciones faltantes y no faltantes. Se puede notar que el patrón de datos faltantes es aleatorio y multivariado, no siendo monótono ni univariado.
...
- Bajo el supuesto de MAR y el patrón observado en los datos faltantes, se sugiere utilizar la imputación múltiple como método de imputación. Se pueden emplear los métodos de Markov Chain Monte Carlo (MCMC) y Fully Conditional Specification (FCS). Estos métodos requieren conocimiento avanzado de estadística.
